<div align="left">
<h1>👋 Hi, I’m Fareed Ologundudu</h1>

<p>
  <strong>Python Data Analyst | Operations & Business Analytics (Remote)</strong>
</p>

<p>
  I help teams identify and fix <strong>operational inefficiencies</strong> by analyzing messy, real-world data and translating it into
  <strong>clear decisions managers can act on immediately</strong>.<br>
  Experience across <strong>Logistics, Sales Operations, Healthcare, and Backend Analytics</strong>.
</p>
</div>

---

## ⚡ What I Actually Do

* Identify where operational processes break (delivery delays, no-shows, revenue leakage)
* Analyze large, imperfect datasets using Python & SQL
* Isolate the small set of issues causing most failures (80/20 analysis)
* Deliver clearly documented insights that reduce management overhead in remote teams

---

## 🚀 Selected Work (Start Here)

### 🏥 Medical Appointment No-Show Prediction

Analyzed **110k+ medical appointments** to identify drivers of patient absenteeism and built a **cost-sensitive ML model** to reduce missed appointments and operational waste.  
Focused on feature behavior, model evaluation trade-offs, and decision impact for healthcare operations.  
🔗 [https://github.com/Fareed04/medical-appointment-no-shows-analysis](https://github.com/Fareed04/medical-appointment-no-shows-analysis)

---

### 📦 Logistics Performance Analysis

Identified a **“double loss”** where late deliveries increased freight costs by **12%** and dropped customer satisfaction to **2.56/5**.
Focused on São Paulo and Rio de Janeiro hubs to surface targeted, actionable operational fixes.
🔗 [https://github.com/Fareed04/logistics-performance-analysis](https://github.com/Fareed04/logistics-performance-analysis)

---

### 🛒 Sales & Revenue Performance Analysis

Analyzed **$2.2M+** in sales data to identify **Technology and Phones** as primary revenue drivers and highlight underperforming segments.
🔗 [https://github.com/Fareed04/sales-performance-analysis](https://github.com/Fareed04/sales-performance-analysis)

---

### 🏥 Healthcare Translation System

Built a real-time speech-to-text translation system using a **Python/Django backend**, demonstrating applied ML in a production-style workflow.
🔗 [https://github.com/Fareed04/healthcare_translation_project](https://github.com/Fareed04/healthcare_translation_project)

---

## 🛠️ Tech Stack

<div>
  <img src="https://skillicons.dev/icons?i=py,django,flask,postgres,mysql,sqlite,git,github,vscode,pycharm,postman,vercel,html,css,js" />
</div>

**Analytics & Data**
Python • SQL • Pandas • NumPy • Matplotlib • Seaborn • Scikit-Learn (applied)

**Backend & Data Access**
Django • Flask • REST APIs • PostgreSQL • MySQL • SQLite

**Workflow & Delivery**
Git • GitHub • Jupyter • VS Code • PyCharm • Postman • Vercel

---

## 🎮 Beyond Work

* ♟️ Chess player — applying strategic thinking and pattern recognition to analytical problem-solving
* I enjoy building systems that are both **useful** and **well-designed**

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Fareed04/Fareed04/output/pacman-contribution-graph-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Fareed04/Fareed04/output/pacman-contribution-graph.svg">
  <img alt="pacman contribution graph" src="https://raw.githubusercontent.com/Fareed04/Fareed04/output/pacman-contribution-graph.svg">
</picture>

---

## 📬 Connect

* LinkedIn: [https://linkedin.com/in/fareed-ologundudu-129506249](https://linkedin.com/in/fareed-ologundudu-129506249)
* GitHub: [https://github.com/Fareed04](https://github.com/Fareed04)
* Email: [ologundudufareed@gmail.com](mailto:ologundudufareed@gmail.com)
